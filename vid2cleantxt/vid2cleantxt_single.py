...
from v2ct_utils import check_runhardware, create_folder, move2completed, \
...
if __name__ == "__main__":
    ...
    filepath = str(input("\n Enter full path to the video to be transcribed ---->"))  # Ask user for folder
    ...
    t_results = transcribe_video_wav2vec(transcription_model=model, directory=directory, vid_clip_name=filename,
    ...
    PL_out = spellcorrect_pipeline(out_p_tscript, transcribed_filename, ns_checker=checker)
    ...
    qk_df.to_csv(join(out_p_tscript, "YAKE - keywords for {}.csv".format(beautify_filename(filename_for_keywords))),
                             index=True)
