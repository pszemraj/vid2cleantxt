...
def transcribe_video_wav2vec(transcription_model, directory, vid_clip_name, chunk_length_seconds, verbose=False):
    ...
    for audio_chunk in tqdm(chunk_directory, total=len(chunk_directory), desc="Transcribing {}".format(vid_clip_name)):
...
if __name__ == "__main__":
    ...
    st = time.time()
    # load audio
    directory = join(os.getcwd(), "example_JFK_speech", "TEST_singlefile")  # finds file based on root of clone loc
    # enter video clip name
    video_file_name = "President John F. Kennedy's Peace Speech.mp4"
    ...
    out_p_tmeta = storage_locs.get('m_out')

    # label and store this transcription
    vid_preamble = beautify_filename(video_file_name, num_words=15,
    ...
    transcribed_file = open(join(out_p_tscript, transcribed_filename), 'w', encoding="utf-8",
                            errors='ignore')
    ...
    metadata_file = open(join(out_p_tmeta, metadata_filename), 'w', encoding="utf-8", errors='ignore')
    ...
    corr_results_fl = symspell_file(filepath=out_p_tscript, filename=transcribed_filename,
    ...
    # ----------------------------------- END -------------------------------
    print("\n\n----------------------------------- Script Complete -------------------------------")
    print("Transcription file + more can be found here: ", out_p_tscript)
    print("Metadata for each transcription is located: ", out_p_tmeta)
