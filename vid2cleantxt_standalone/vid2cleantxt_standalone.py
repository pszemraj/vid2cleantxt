...
def timelog_analytics(
    ...
    if save_plot:
        fig.to_html(
            join(
                out_p_metadata, "transcriptions {} run time viz.html".format(tag_date)
            ),
...
if __name__ == "__main__":
    ...
    for filename in tqdm(
        ...
        print(
            "transcribed {} in {} minutes".format(
                shorten_title(filename), round(this_rt, 2)
            )
        )
    ...
    if merge_transc:
        ...
        digest_txt_directory(
            out_p_metadata,
            identifer="_metadata_for_tscript_run" + t_merge,
            make_folder=False,
        )
    else:
    ...
    for origi_tscript in tqdm(
        ...
        PL_out = spellcorrect_pipeline(
            out_p_tscript, origi_tscript, ns_checker=checker, verbose=False
        )
